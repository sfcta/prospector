...
const CUSTOM_BP_DICT = {
  'total': {'base':[250, 500, 750, 1000], 'diff':[-100, -5, 5, 100], 'pctdiff':[-20, -5, 5, 20]},
  'cie_avg': {'base':[250, 500, 750, 1000], 'diff':[-100, -5, 5, 100], 'pctdiff':[-20, -5, 5, 20]},
  'med_avg': {'base':[250, 500, 750, 1000], 'diff':[-100, -5, 5, 100], 'pctdiff':[-20, -5, 5, 20]},
  'mips_avg': {'base':[250, 500, 750, 1000], 'diff':[-100, -5, 5, 100], 'pctdiff':[-20, -5, 5, 20]},
  'pdr_avg': {'base':[250, 500, 750, 1000], 'diff':[-100, -5, 5, 100], 'pctdiff':[-20, -5, 5, 20]},
  'retail_avg': {'base':[250, 500, 750, 1000], 'diff':[-100, -5, 5, 100], 'pctdiff':[-20, -5, 5, 20]},
  'visitor_avg': {'base':[250, 500, 750, 1000], 'diff':[-100, -5, 5, 100], 'pctdiff':[-20, -5, 5, 20]},
}
...
async function getMapData() {
  ...
  for (let entry of jsonData) {
    ...
    for (let yr of YR_LIST) {
      for (let mode of TRANSIT_MODE) {
        for (let met of app.metric_options) {
          tmp[yr][mode][met.value] += entry[mode+'_'+met.value];
        }
      }
    }
  }
...
async function drawMapFeatures(queryMapData=true) {
  ...
  if (app.isTRActive == true && app.isAUActive == false) {
    base_metric = 'transit_' + sel_metric;
    comp_metric = 'transit_' + sel_metric;
  }
  if (app.isAUActive == true && app.isTRActive == false) {
    base_metric = 'auto_' + sel_metric;
    comp_metric = 'auto_' + sel_metric;
  }
  ...
  try {
      ...
      for (let feat of cleanFeatures) {
        ...
        if (app.comp_check) {
          ...
          if (base_lookup.hasOwnProperty(feat.taz)) {
            ...
            // console.log(base_lookup.hasOwnProperty(feat.taz))
    ...
    if (map_vals.length > 0) {
      ...
      if (queryMapData) {
        ...
        // console.log(INT_COLS.includes(sel_metric))
...
let app = new Vue({
  ...
  data: {
    ...
    selected_metric: 'cie_avg',
    metric_options: [
    {text: 'Total_Jobs', value: 'total'},
    {text: 'CIE_Jobs', value: 'cie_avg'},
    {text: 'MED_Jobs', value: 'med_avg'},
    {text: 'MIPS_Jobs', value: 'mips_avg'},
    {text: 'PDR_Jobs', value: 'pdr_avg'},
    {text: 'RET_Jobs', value: 'retail_avg'},
    {text: 'VIS_Jobs', value: 'visitor_avg'},
    ],
